...
def sales(initial_date: datetime = None, final_date: datetime = None, client_id: int = None):
...
def products_with_sales():
    sales_with_items_df = sales_df.set_index("id").join(sales_items_df.set_index("sale_id"))
...
def total_sales_by_products(initial_date: datetime, final_date: datetime):
    filtered_sales_df = sales(initial_date, final_date)
    sales_with_items_df = filtered_sales_df.set_index("id").join(sales_items_df.set_index("sale_id"))
    ...
    sales_with_items_df = sales_with_items_df.join(products_df[["id", "name"]].set_index("id"))
...
def total_sales_by_client():
    ...
    sales_with_items_df = sales_with_items_df.join(clients_df[["id", "name"]].set_index("id"))
    sales_with_items_df.reset_index(inplace=True)
